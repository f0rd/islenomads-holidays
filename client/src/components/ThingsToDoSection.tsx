...
export default function ThingsToDoSection({ activities, islandName }: ThingsToDoSectionProps) {
  ...
  return (
    <div className="w-full space-y-6">
      {/* Section Header - Smaller to match Getting There */}
            ...
            <div className="relative overflow-hidden rounded-xl border border-border/40 bg-gradient-to-br from-card/60 via-card/40 to-card/20 backdrop-blur-sm transition-all duration-300 hover:border-accent/40 hover:shadow-xl hover:shadow-accent/10">
              ...
              <div className="relative p-6 md:p-8 flex items-start gap-6">
                ...
                <div className="flex items-start gap-5 flex-1 min-w-0">
                  {/* Modern Number Badge */}
                  <div className="flex-shrink-0 w-12 h-12 rounded-lg bg-gradient-to-br from-accent to-accent/80 text-white flex items-center justify-center font-bold text-lg shadow-lg shadow-accent/30 group-hover:scale-110 group-hover:shadow-xl group-hover:shadow-accent/40 transition-all duration-300">
                    {index + 1}
                  </div>

                  {/* Title and Meta */}
                  <div className="flex-1 min-w-0 pt-1">
                    <h4 className="text-lg md:text-xl font-bold text-foreground group-hover:text-accent transition-colors duration-300">
                      {activity.title}
                    </h4>
                    ...
                    <div className="flex flex-wrap gap-2 items-center mt-2">
                      {activity.duration && (
                        <span className="text-xs md:text-sm bg-blue-500/15 text-blue-600 px-2 py-0.5 rounded-full border border-blue-200/50 font-medium">
                          ⏱️ {activity.duration}
                        </span>
                      )}
                      {activity.difficulty && (
                        <span
                          className={`text-xs md:text-sm px-2 py-0.5 rounded-full font-medium ${getDifficultyColor(
                            activity.difficulty
                          )}`}
                        >
                          📊 {activity.difficulty}
                      ...
                      {activity.bestTime && (
                        <span className="text-xs md:text-sm bg-purple-500/15 text-purple-600 px-2 py-0.5 rounded-full border border-purple-200/50 font-medium">
                          🗓️ {activity.bestTime}
                        </span>
                      )}
                    </div>
            ...
            <div className="mt-2 rounded-xl border border-accent/20 bg-gradient-to-br from-accent/8 via-accent/5 to-transparent backdrop-blur-sm p-6 md:p-8 space-y-6">
              ...
              {activity.tips && activity.tips.length > 0 && (
                <div className="bg-gradient-to-r from-accent/10 to-accent/5 border-l-4 border-accent p-4 rounded-lg space-y-3">
                  <h5 className="font-semibold text-foreground flex items-center gap-2 text-base">
                    💡 Practical Tips
                  </h5>
